...
/**
 * PGStream implementation using regular Stream-based sockets
 */
public class PGRegularStream extends AbstractPGStream {
     ...
     */
    public PGRegularStream(String host, int port) throws IOException
    {
        super(host, port);
    ...
    public PGStream reconnect(boolean closeOriginal) throws IOException {
        ...
        return new PGRegularStream(getHost(), getPort());
    }

    public StreamReadListener readListener(StreamReadListener newListener) {
        return null;
    }

    public void addTimerTask(final Runnable timerTask, long milliSeconds) {
        Driver.addTimerTask(new TimerTask() {
            public void run() {
                timerTask.run();
            }
        }, milliSeconds);
